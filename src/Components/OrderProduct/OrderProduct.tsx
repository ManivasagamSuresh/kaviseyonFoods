...
const OrderProduct: React.FC<OrderProductProps> = ({ order, setOrders }) => {
  ...
  const handleSaveOrderStatus = async () => {
    try {
      ...
      const payload = {
        ...
        trackingId: orderState.orderStatus === "Shipped" && orderState.trackingId,
      };
  ...
  return (
    <div className="border rounded-lg shadow-lg">
      <div className="top bg-themeColorLight px-3 md:px-5 2xl:px-16 py-3 flex justify-between rounded-t-lg">
          ...
          <div className="flex gap-8">
            ...
            <div className="flex flex-row text-xs md:text-base gap-0 md:gap-1 h-fit items-center">
              ...
              <span className="flex items-center ">
                <LiaRupeeSignSolid className="text-xs ml-2 md:ml-0" /> {orderState.orderTotal}
              </span>
            </div>
          </div>
          <div className="w-full  text-[10px] md:text-base md:hidden">
            <div>Order Id: {orderState._id}</div>
            <div>Payment Id: {orderState.razorpay_paymentId}</div>
          </div>
        </div>
        <div className="text-[10px] md:text-base hidden md:block w-full  md:w-64">
          <div>Order Id: {orderState._id}</div>
          <div>Payment Id: {orderState.razorpay_paymentId}</div>
        </div>
      </div>
      <div className="flex flex-col gap-2 py-4 pb-7 px-3 md:px-5 2xl:px-16">
                ...
                {orderState.orderStatus !== "Delivered" && (
                  <div className="text-sm w-64 overflow-hidden">
                    {orderState.trackingId && orderState.orderStatus === "Shipped" ? (
                      <a
                        href={`${orderState.trackingId}`}
                        target="_blank"
                        className="underline"
                      >{`Tracking Link: ${orderState.trackingId}`}</a>
                    ) : (
              ...
              <div>
                ...
                {orderState.orderStatus === "Shipped" && (
                  <div className="text-sm md:text-base ">
                    ...
                    <input
                      type="text"
                      className="border-b border-themeColorDark px-4 py-1"
                      onChange={updateTrackingId}
                      value={orderState.trackingId ? orderState.trackingId : ""}
                    />
                  </div>
                )}
              </div>
            )}
            {kaviFoodUser?.isAdmin && editmode === false ? (
              <TbEdit className="cursor-pointer" onClick={() => setEditMode(true)} />
            ) : (
              <></>
            )}
            {kaviFoodUser?.isAdmin && editmode ? (
              <LuSave className="cursor-pointer" onClick={() => handleSaveOrderStatus()} />
            ) : (
              <></>
            )}
          </div>
        ...
        <div className="flex justify-between md:flex-row flex-col gap-2 md:gap-0">
              ...
              return (
                <div className="flex gap-4" key={`${prod.productId}`}>
                  ...
                  <div className="flex flex-col gap-1">
                    <div>
                      {prod.name}{" "}
                      <span className="text-xs text-lightGrey ml-2">
                        {prod.weight_in_grams}gms{" "}
                      </span>
                    </div>
          ...
          <div className="text-sm w-full  md:w-64">
            ...
            <div className="flex md:flex-col flex-wrap">
              ...
              <span className="flex flex-wrap">{`${orderState.deliveryAddress.address} ${orderState.deliveryAddress.city} - ${orderState.deliveryAddress.pincode} ${orderState.deliveryAddress.state}`}</span>{" "}
            </div>

            <div>
              {" "}
              <span className="font-semibold"> Contact No: </span> {orderState.mobile}
            </div>
            <div>
              {" "}
              <span className="font-semibold"> Email: </span> {orderState.email}
            </div>
          </div>
        </div>
      </div>
