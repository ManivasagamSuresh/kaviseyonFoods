...
const handleAddCart = async (_id: any, cartItem: any) => {
  try {
    ...
    if (isAdded) {
      ...
      const increaseTotal = await db
        ?.collection("user")
        .updateOne(
          { _id: new ObjectId(`${_id}`) },
          { $inc: { "cart.totalPrice": cartItem.price } }
        );
      await closeConnection();
    ...
    const addCart = await db
      ?.collection("user")
      .updateOne({ _id: new ObjectId(`${_id}`) }, { $push: { "cart.items": cartItem } });
    const increaseTotal = await db
      ?.collection("user")
      .updateOne({ _id: new ObjectId(`${_id}`) }, { $inc: { "cart.totalPrice": cartItem.price } });
    // Check if the update operation was successful
  ...
  try {
    ...
    const removeResult = await db
      ...
      .updateOne({ _id: new ObjectId(`${_id}`) }, {
        $pull: { "cart.items": { _id: cartItem._id } },
      } as any);
    const decreaseTotal = await db
      ?.collection("user")
      .updateOne(
        { _id: new ObjectId(`${_id}`) },
        { $inc: { "cart.totalPrice": -(cartItem.price * cartItem.quantity) } }
      );
...
const handleEmptyCart = async (_id: string) => {
  try {
    ...
    const emptyCart = await db
      ?.collection("user")
      .updateOne({ _id: new ObjectId(`${_id}`) }, { $set: { cart: { totalPrice: 0, items: [] } } });
