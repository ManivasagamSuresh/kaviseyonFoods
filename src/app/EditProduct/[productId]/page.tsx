...
function Page() {
  ...
  const editProduct = async (prod: Product) => {
    try {      
      const response = await axios.put(`/api/productsAPi?productId=${params.productId}`, prod);
  ...
  const getProductDetails = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/api/productsAPi?productId=${params.productId}`);
      ...
      setFormValues({
        ...
        weight_in_grams: productDetails.weight_in_grams,
  ...
  return (
    <>
   {loading ? <div className="h-screen"> <ClipLoader loading={loading} color="#fff" size={18} /> </div > :  <div className="w-full max-w-xl mx-auto p-6 bg-white shadow-lg rounded-md my-20">
      <h1 className="text-2xl font-semibold mb-6 text-center text-themeColorDark">Edit Product</h1>
